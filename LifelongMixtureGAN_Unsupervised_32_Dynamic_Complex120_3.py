...
class LifeLone_MNIST(object):
    def __init__(self):
        ...
        self.mnist_train_y = np.zeros((np.shape(x_train)[0], 7))
        ...
        self.mnist_test_y = np.zeros((np.shape(mnist_test)[0], 7))
        self.mnist_test_y[:, 0] = 1

        self.svhn_train_x = x_train
        self.svhn_train_y = np.zeros((np.shape(x_train)[0], 7))
        self.svhn_train_y[:, 1] = 1
        self.svhn_label = y_train
        self.svhn_label_test = y_test
        self.svhn_test_x = x_test
        self.svhn_test_y = np.zeros((np.shape(x_test)[0], 7))
        ...
        # load 3D chairs
        img_path2 = glob('../CACD2000/CACD2000/*.jpg')  # 获取新文件夹下所有图片
        ...
        img_path = glob('../train_64x64/*.png')  # 获取新文件夹下所有图片
        data_files = img_path
        data_files = sorted(data_files)
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            for taskIndex in range(taskCount):
                if taskIndex == 0:
                    currentTrainX = self.CelebaTrainX
                    currentTrainX = self.cifar_train_x
                elif taskIndex == 1:
                    currentTrainX = self.CACDTrainX
                    currentTrainX = self.ImageNet1_trainX
                elif taskIndex == 2:
                    currentTrainX = self.cifar_train_x
                elif taskIndex == 3:
                    currentTrainX = self.ImageNet1_trainX
                ...
                for epoch in range(start_epoch, self.epoch):
                    ...
                    ims("results/" + "Test_Test_" + str(epoch) + ".png", merge2(gan[:64], [8, 8]))

                nextTaskIndex = taskIndex + 1
                self.Select_Expert(1)
                self.componentCount = 1
            ...
            for k in range(np.shape(self.GeneratorArr)[0]):
                ...
                ims("results/" + "LTS_Complex_1_GAN" + str(k) + ".png", merge2(gan[:64], [8, 8]))

            '''
            ssim1, psnr1, reco1, elbo1 = self.Evaluation(self.CelebaTestX)
            ssim2, psnr2, reco2, elbo2 = self.Evaluation(self.CACDTestX)
            ssim3, psnr3, reco3, elbo3 = self.Evaluation(self.cifar_test_x)
            ssim4, psnr4, reco4, elbo4 = self.Evaluation(self.ImageNet1_testX)
            ssim5, psnr5, reco5, elbo5 = self.Evaluation(self.svhn_test_x)
            ssim6, psnr6, reco6, elbo6 = self.Evaluation(self.mnist_test_x)

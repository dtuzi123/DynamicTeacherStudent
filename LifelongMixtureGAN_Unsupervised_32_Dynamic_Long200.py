...
def Image_classifier(inputs, scopename, is_training=True, reuse=False):
    with tf.variable_scope(scopename, reuse=reuse):
        ...
        with slim.arg_scope([slim.conv2d, slim.fully_connected],
            ...
            outputs = slim.fully_connected(net, 7, activation_fn=None, normalizer_fn=None, scope='fco')
    return outputs
...
class LifeLone_MNIST(object):
    def __init__(self):
        ...
        self.InverseMNISTTrain_x, self.InverseMNISTTrain_label, self.InverseMNISTTest_x, self.InverseMNISTTest_label = Give_InverseMNIST32()
        ...
        currentTrainX = data_files

        batchFiles2 = zimuFiles
        ...
        batch2 = np.reshape(batch2, (-1, 32, 32, 1))
        batch_images = np.concatenate((batch2, batch2, batch2), axis=-1)
        startIndex = np.shape(batch_images)[0]-10000
        self.zimu = batch_images[0:startIndex]
        self.zimuTest = batch_images[startIndex:startIndex+10000]
        self.GeneratorArr = []
    ...
    def build_model(self):
        ...
        self.y = tf.placeholder(tf.float32, [self.batch_size, 7])
        self.labels = tf.placeholder(tf.float32, [self.batch_size, 10])
        self.weights = tf.placeholder(tf.float32, [self.batch_size,7])
        self.index = tf.placeholder(tf.int32, [self.batch_size])
        self.gan_inputs = tf.placeholder(tf.float32, [bs] + image_dims)
        self.gan_domain = tf.placeholder(tf.float32, [self.batch_size, 7])
        ...
        with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
            ...
            '''
            self.d_optim1 = tf.train.RMSPropOptimizer(learning_rate=0.0004) \
                .minimize(self.d_loss1, var_list=discriminator_vars1)
            self.g_optim1 = tf.train.RMSPropOptimizer(learning_rate=0.0004) \
                .minimize(self.g_loss1, var_list=GAN_generator_vars1)
    ...
    def GiveReconstruction(self,batchImage):
        G_all = self.gan_inputs
        z_dim = 150
        # encoder continoual information
        ...
        code = tf.concat((continous_variables1, discrete_real), axis=1)
        VAE1 = Generator_SVHN("VAE_Generator", code, reuse=True)
        reco = self.sess.run(VAE1, feed_dict={self.gan_inputs: batchImage})
        return reco
    ...
    def Calculate_FID_Score(self, nextTaskIndex):
        ...
        if nextTaskIndex == 0:
            ...
            nextTrainX = self.cifar_train_x
        elif nextTaskIndex == 5:
            nextTrainX = self.zimu
        elif nextTaskIndex == 6:
            nextTrainX = self.mnist_train_x
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            # saver to save model
            self.saver = tf.train.Saver()
            ExpertWeights = np.ones((self.batch_size, 7))
            ...
            for taskIndex in range(taskCount):
                if taskIndex == 0:
                    ...
                    currentTrainX = self.zimu
                elif taskIndex == 6:
                    currentTrainX = self.mnist_train_x

                currentY = np.zeros((np.shape(currentTrainX)[0], 7))
                    ...
                    for idx in range(start_batch_id, self.num_batches):
                        ...
                        if idx % 5 == 0:
                            ...
                            _, g_loss = self.sess.run(
                                ...
                                feed_dict={self.inputs: batch_images, self.z: batch_z, self.y: batch_y, self.gan_inputs: batch_images})

                        if taskIndex > 0:
                            ganSamples = batch_images
                            gan_domain = np.zeros((self.batch_size, 7))
                            gan_domain[:, self.currentComponent-1] = 1

                            batch_z = np.random.uniform(-1, 1, [self.batch_size, self.z_dim]).astype(np.float32)
                            for i in range(np.shape(self.GeneratorArr)[0]):
                                newGan_domain = np.zeros((self.batch_size, 7))
                                newGan_domain[:, i] = 1
                            ...
                            ganSamples = batch_images
                            gan_domain = np.zeros((np.shape(ganSamples)[0], 7))
                            gan_domain[:, 0] = 1
                            gan_domain_labels = [np.argmax(one_hot) for one_hot in gan_domain]
            ...
            for k in range(np.shape(self.GeneratorArr)[0]):
                ...
                ims("results/" + "TeacherLong" + str(k) + ".png", merge2(gan[:64], [8, 8]))

            end_time = time.time()
            runingTime = end_time - start_time
            print("running times")
            print(runingTime)

            test1 = self.mnist_test_x[0:20]
            test2 = self.svhn_test_x[0:20]
            test3 = self.FashionTest_x[0:20]
            test4 = self.InverseFashionTest_x[0:20]
            test5 = self.cifar_test_x[0:20]
            test6 = self.zimuTest[0:20]

            test = np.concatenate((test1,test2,test3,test4,test5,test6),axis=0)

            n_examples = np.shape(test)[0]
            index = [i for i in range(n_examples)]
            random.shuffle(index)
            test = test[index]
            test = test[0:64]
            ganDomain = np.zeros((self.batch_size,7))
            ganDomain[:,0] = 1
            ims("results/" + "StudentLong" + ".png", merge2(test[:64], [8, 8]))
            reco = self.GiveReconstruction(test)
            ims("results/" + "StudentLongReco" + ".png", merge2(reco[:64], [8, 8]))
            ...
            ssim5, psnr5, reco5, elbo5 = self.Evaluation(self.cifar_test_x)
            ssim6, psnr6, reco6, elbo6 = self.Evaluation(self.zimuTest)

            ssimSum = ssim1 + ssim2 + ssim3 + ssim4 + ssim5 + ssim6
            psnrSum = psnr1 + psnr2 + psnr3 + psnr4 + psnr5 + psnr6
            recoSum = reco1 + reco2 + reco3 + reco4 + reco5 + reco6
            elboSum = elbo1 + elbo2 + elbo3 + elbo4 + elbo5 + elbo6
            ...
            print("number of components for the teacher")
            print(np.shape(self.GeneratorArr))

...
class LifeLone_MNIST(object):
    ...
    def build_model(self):
        ...
        with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
            self.d_optim1 = tf.train.RMSPropOptimizer(learning_rate=0.00005) \
                .minimize(self.d_loss1, var_list=discriminator_vars1)
            self.g_optim1 = tf.train.RMSPropOptimizer(learning_rate=0.00005) \
        ...
        with tf.Session(config=config) as sess:
            ...
            for taskIndex in range(taskCount):
                ...
                for epoch in range(start_epoch, self.epoch):
                    ...
                    ims("results/" + "Test_5_" + str(epoch) + ".png", merge2(gan[:64], [8, 8]))

...
class LifeLone_MNIST(object):
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            self.InverseFashionTrain_x = tf.image.flip_left_right(self.InverseFashionTrain_x)
            self.InverseFashionTest_x = tf.image.flip_left_right(self.InverseFashionTest_x)
            self.InverseFashionTrain_x = self.InverseFashionTrain_x.eval()
            self.InverseFashionTest_x = self.InverseFashionTest_x.eval()
            '''
            ...
            for taskIndex in range(taskCount):
                ...
                for epoch in range(start_epoch, self.epoch):
                    ...
                    batch_z = np.random.uniform(-1, 1, [self.batch_size, self.z_dim]).astype(np.float32)
                    gan = self.sess.run(self.GeneratorArr[np.shape(self.GeneratorArr)[0]-1], feed_dict={self.z: batch_z})
            ...
            for k in range(np.shape(self.GeneratorArr)[0]):
                ...
                ims("results/" + "GAN_Complex__2_" + str(k) + ".png", merge2(gan[:64], [8, 8]))
